 ...
 * 收集到一个state对象中，该对象的key与传递的reducer 方法的key相对应。
 *
 ...
 * 一个包含不同reducer函数并且需要组合的对象。 一种方便的获取方式是使用
 ...
 * 一个reducer函数，该函数会遍历调用传递的reducers参数对象内的每个reducer，
 * 并且构建一个具有reducers参数对象相同结构的state对象
 */
